...
USING_NS_CC;
ShaderDemoNode* ShaderDemoNode::createWithContentSize(const cocos2d::Size& nodeSize)
{
    assert(nodeSize.width>0.0f && nodeSize.height>0.0f);
    
    ShaderDemoNode *pRet = new(std::nothrow) ShaderDemoNode;
...
ShaderDemoNode::~ShaderDemoNode()
{
    clearData();
}

bool ShaderDemoNode::initWithContentSize(const cocos2d::Size& nodeSize)
{
...
void ShaderDemoNode::loadShaderPrograme()
{
    GLProgram* pShaderProgram = GLProgram::createWithFilenames("res/shader_practice/case04.vert", "res/shader_practice/case04.frag");
...
void ShaderDemoNode::draw(Renderer* renderer, const Mat4& transform, uint32_t flags)
{
    m_customCommand.init(_globalZOrder, transform, flags);
    m_customCommand.func = CC_CALLBACK_0(ShaderDemoNode::onDraw, this, transform, flags);
