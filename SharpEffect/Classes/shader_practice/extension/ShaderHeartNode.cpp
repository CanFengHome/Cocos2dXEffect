...
USING_NS_CC;
ShaderHeartNode* ShaderHeartNode::createWithContentSize(const cocos2d::Size& nodeSize)
{
    assert(nodeSize.width>0.0f && nodeSize.height>0.0f);
    
    ShaderHeartNode *pRet = new(std::nothrow) ShaderHeartNode;
...
ShaderHeartNode::~ShaderHeartNode()
{
    clearData();
}

bool ShaderHeartNode::initWithContentSize(const cocos2d::Size& nodeSize)
{
...
void ShaderHeartNode::setPosition(const Vec2 &newPosition)
{
...
void ShaderHeartNode::loadShaderPrograme()
{
    GLProgram* pShaderProgram = GLProgram::createWithFilenames("res/shader_practice/case06.vert", "res/shader_practice/case06.frag");
    
    GLProgramState* pProgramState = GLProgramState::getOrCreateWithGLProgram(pShaderProgram);
    setGLProgramState(pProgramState);
}

void ShaderHeartNode::loadData()
{
...
void ShaderHeartNode::clearData()
{
...
void ShaderHeartNode::draw(Renderer* renderer, const Mat4& transform, uint32_t flags)
{
    m_customCommand.init(_globalZOrder, transform, flags);
    m_customCommand.func = CC_CALLBACK_0(ShaderHeartNode::onDraw, this, transform, flags);
    renderer->addCommand(&m_customCommand);
}

void ShaderHeartNode::onDraw(const cocos2d::Mat4& transform, uint32_t flags)
{
    // gl programe use
    ...
    getGLProgramState()->setUniformVec2("resolution", m_resolution);
    getGLProgramState()->setUniformVec2("center", m_center);
